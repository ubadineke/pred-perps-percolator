  ...
  return (
    <div className="terminal-layout">
      <aside className="market-rail">
        ...
        {markets.map((item) => <Link href={`/trade/${item.slug}`} className={item.slug === market.slug ? "active" : ""} key={item.slug}><span><i />{item.short}</span><b>{item.moxie.toFixed(1)}¢</b><em className={item.change===null?"":item.change>=0?"positive":"negative"}>{item.change===null?"LIVE":`${item.change>=0?"+":""}${item.change}%`}</em></Link>)}
      </aside>
      ...
      <section className="trade-workspace">
        <div className="trade-titlebar">
          ...
          <dl><div><dt>MOXIE</dt><dd>{market.moxie.toFixed(1)}¢</dd></div><div><dt>INDEX</dt><dd>{market.index.toFixed(1)}¢</dd></div><div><dt>MARK</dt><dd>{market.mark.toFixed(1)}¢</dd></div><div><dt>24H</dt><dd>{market.change===null?"—":`${market.change>=0?"+":""}${market.change}%`}</dd></div><div><dt>LOCKS IN</dt><dd className="warning"><Clock3 size={12} /> {market.lock}</dd></div></dl>
        </div>
        <div className="chart-panel">
          ...
          <div className="chart-area">
            <div className="chart-price"><strong>{market.moxie.toFixed(1)}¢</strong><span>Current indexed mark</span></div>
        ...
        <div className="lower-panel">
          <div className="panel-tabs"><button className="active" type="button">Positions <span>0</span></button><button type="button">Open orders</button><button type="button">Trade history</button><button type="button">Funding / premium</button></div>
          <div className="empty-activity"><div><b>No portfolio loaded</b><span>Connect a portfolio to display its indexed positions here.</span></div></div>
        </div>
      </section>
      ...
      <aside className="order-ticket">
        ...
        <div className="balance-line"><span>Collateral preview</span><b>Not connected</b></div>
        ...
        <div className="quick-percent"><button onClick={() => setCollateral(250)} type="button">$250</button><button onClick={() => setCollateral(500)} type="button">$500</button><button onClick={() => setCollateral(750)} type="button">$750</button><button onClick={() => setCollateral(1000)} type="button">$1K</button></div>
